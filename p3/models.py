...
class Encoder(nn.Module):
    ...
    def forward(
        ...
        #############################################
        # Hint: At each time-step, the LSTM decoder receives as input the embedding of the 
        # current word (self.embedding), then each output at each timestep has dropout applied 
        # (self.dropout) and the resulting decoder state is used in the next timestep.
        ...
        # Returns: a :class:`PackedSequence` object
        packed_embedding = pack(input = embedding, lengths = lengths, batch_first = True, enforce_sorted = False)

        packed_embedding_out, final_hidden = self.lstm(packed_embedding)
        
        # - Use torch.nn.utils.rnn.pad_packed_sequence to unpack the packed sequences
        #   (after passing them to the LSTM)
        # Returns:Tuple of Tensor containing the padded sequence, and a Tensor
        # containing the list of lengths of each sequence in the batch.
        # Batch elements will be re-ordered as they were ordered originally when
        # the batch was passed to ``pack_padded_sequence`` or ``pack_sequence``.
        enc_output, _lists_len_each_seq = unpack(sequence = packed_embedding_out, batch_first = True)
...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        # src_lengths: (batch_size)
        # bidirectional encoder outputs are concatenated so we need to reshape the decoder states to be of size (num_layers, batch_size, 2*hidden_size)
        ...
        # tgt_embedded: (batch_size, max_tgt_len, hidden_size)
